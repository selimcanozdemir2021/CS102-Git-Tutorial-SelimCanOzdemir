...
public class Cipher   
{
    ...
    public String encrypt(String inputString) {
        ...
        {
            char current = inputString.charAt(i); // determining the current char to change it
            char willBeAdded = replaceChar(current, true); // changing the char to the encrypted version (isEncrypt true)

            outputString = outputString + willBeAdded;
        }
    ...
    public String decrypt(String inputString) {
       ...
       for (int i = 0; i < inputString.length(); i++)   
        {
            char current = inputString.charAt(i); // determining the current char to change it
            char willBeAdded = replaceChar(current, false); // changing the char to the decrypted version (isEncrypt false)
        ...
        if(isEncrypt) {
            for (int i = 0; i < ORIGINAL_ALPHABET.length(); i++)   
            {
                if(ORIGINAL_ALPHABET.charAt(i) == inputChar) {
                    return CIPHER_ALPHABET.charAt(i); // missing part for encrypting
                }
            }
        }
        else {
